            ...
            <template #header>
              <el-tooltip :content="$t('audit.selectAllFiltered')">
                <el-checkbox
                  ...
                  :disabled="reviewLocked || filteredActionableIds.length === 0"
                  @change="toggleAllFiltered"
                />
              </el-tooltip>
            </template>
            <template #default="scope">
              <el-tooltip :content="scope.row.action ? $t('audit.previousReportReadOnly') : $t('audit.noApprovableAction')" :disabled="!reviewLocked && Boolean(scope.row.action)">
                <span class="anomaly-checkbox">
                  <el-checkbox
                    :model-value="selectedIdSet.has(scope.row.id)"
                    :disabled="reviewLocked || !scope.row.action || report.executable === false"
                    @click.stop
...
import { Connection, FolderOpened, Picture, Search, TopRight, VideoPlay } from '@element-plus/icons-vue'
...
const emit = defineEmits(['start', 'verify-conflicts', 'update:actionIds'])
const { t, te } = useI18n()
...
@media (max-width: 900px) { .task-toolbar, .task-toolbar-main { align-items: flex-start; flex-wrap: wrap; }.filter-row { grid-template-columns: 1fr; }.anomaly-detail-grid.has-book-preview { grid-template-columns: minmax(0, 1fr); } }
</style>
